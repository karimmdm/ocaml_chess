    ...
    | None ->
        if String.equal clr "white" then
          if String.equal diag "main" then
            ...
            check_diag grid clr (x - 1) (y + 1) ((x, y) :: acc) diag
            @ check_diag grid clr (x + 1) (y - 1) ((x, y) :: acc) diag
        else if String.equal diag "main" then
          check_diag grid clr (x + 1) (y - 1) ((x, y) :: acc) diag
          @ check_diag grid clr (x - 1) (y + 1) ((x, y) :: acc) diag
        else
          check_diag grid clr (x - 1) (y - 1) ((x, y) :: acc) diag
...
(* [check_rank board clr x y acc dir] returns [acc], a list of valid
   positions in the row or column, specified by [dir] starting at (x,
   y). *)
...
let locations st p =
  ...
  match piece with
  | Pawn ->
      (* [pawn_capture board clr x y] returns a list of positions that
         the given pawn piece can move to after a legal capture. *)
      let pawn_capture board clr x y =
        ...
        if String.equal clr "white" then
          ...
          helper check_locs []
      in
      (* [pawn_move board clr x y] returns a list of positions that the
         given pawn piece can move to, excluding captures. *)
  ...
  | Bishop ->
      (* [bishop_move board clr x y] returns a list of positions that
         the given bishop piece can move to, including captures. *)
      let bishop_move board clr x y =
        check_diag board clr x y [] "main"
        @ check_diag board clr x y [] "snd"
      in
      bishop_move board clr x y
  | Knight ->
      (* [knight_move board clr x y] returns a list of positions that
         the given knight piece can move to, including captures. *)
      let knight_move board clr x y =
        ...
        in
        valid_positions board clr x y check_locs []
      in
      knight_move board clr x y
  | Rook ->
      (* [rook_move board clr x y] returns a list of positions that the
         given rook piece can move to, including captures. *)
      let rook_move board clr x y =
        check_rank board clr x y [] "ver"
        @ check_rank board clr x y [] "hor"
      in
      rook_move board clr x y
  | Queen ->
      (* [queen_move board clr x y] returns a list of positions that the
         given queen piece can move to, including captures. *)
